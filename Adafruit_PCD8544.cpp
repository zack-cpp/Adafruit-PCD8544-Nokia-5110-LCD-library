...
void Adafruit_PCD8544::printAlignmentCenter(String dataString, byte xSize, byte ySize, byte spacee){
  String header[3] = {"", "", ""};
  String footer[3] = {"", "", ""};
...
void Adafruit_PCD8544::printAlignmentCenter(String dataString, String headerOrFooter[], byte xSize, byte ySize, byte spacee, bool noFooter){
  if (noFooter){
    String footer[3] = {"", "", ""};
...
void Adafruit_PCD8544::printAlignmentCenter(String dataString, String header[], String footer[], byte xSize, byte ySize, byte spacee){
  //  parse raw data
  byte i = 0;
  byte dataNow = 0;
  byte jumlahData = 1;
  ...
  int xCoord[jumlahData];
  int yCoord[jumlahData];
  ...
  for (byte i = 0; i < jumlahData; i++){
    ...
    // if (text[i].length() < LCDWIDTH - (6 * xSize)){
      
    // }
    xCoord[i] = (LCDWIDTH - (text[i].length() * 6 * xSize)) / 2;
    if(jumlahData != 1){
      yCoord[i] = (LCDHEIGHT - (jumlahData * 8 * ySize + (jumlahData * spacee - spacee))) / 2 + (i * (8 * ySize + spacee));
    }else{
      yCoord[i] = (LCDHEIGHT - (jumlahData * 8 * ySize)) / 2 + (i * (8 * ySize + spacee));
    }
